...
namespace jump {
template <typename T>
GeneralisedEigenvalueSystem<T>::GeneralisedEigenvalueSystem(DenseMatrix<T>& A,
        DenseMatrix<T>& B, std::vector<Eigendatum<T>>& data) :
    m_A{A},
    m_B{B},
    m_data{data} {
#ifndef NDEBUG
...
template <typename T>
auto GeneralisedEigenvalueSystem<T>::order() const -> std::size_t {
    return m_A.num_rows();
}
