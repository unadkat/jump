...
namespace Jump {
    ...
    ///
    /// Any time we open a file (be it for input or output), we must supply
    /// an explicit access specifier so that no files are accidentally
    /// overwritten or written to instead of read from, etc. Files can be opened
    /// for input or output (or both), and in the case of output they can be
    /// opened for appending data or truncation (overwriting). This gives the
    ...
    /// ensures that files are closed appropriately when finished with.
    class FileSystem {
        private:
        ...
        public:
            /// \brief Use current working directory as root folder for all
            /// input and output files opened by this object, optionally take
            /// a subdirectory location to be used.
            ...
            /// directory), with a given handle, and an access specifier taken
            /// from the FileMode namespace. Output precision may also
            /// optionally be set.
            ...
            /// \brief Open file with a specified filename (relative to the root
            /// directory) for quick data reading, without explicitly storing
            /// the file handle for later access.
            std::fstream quick_read(const std::string& filename) const;

            /// \brief Return file stream corresponding to the supplied handle,
