...
#ifndef JUMP_MAT4_HPP
#define JUMP_MAT4_HPP
...
#include "jump/experimental/data/vec4.hpp"
...
class Mat<T, 4> {
    ...
    public:
        ...
        constexpr Mat(const ValueType& a11, const ValueType& a22,
                const ValueType& a33, const ValueType& a44);
        constexpr Mat(const ValueType& a11, const ValueType& a21,
                const ValueType& a31, const ValueType& a41,
                const ValueType& a12, const ValueType& a22,
                const ValueType& a32, const ValueType& a42,
                const ValueType& a13, const ValueType& a23,
                const ValueType& a33, const ValueType& a43,
                const ValueType& a14, const ValueType& a24,
                const ValueType& a34, const ValueType& a44);
...
template <typename T>
inline constexpr Mat<T, 4>::Mat(const ValueType& value) :
    m_storage{value, 0, 0, 0, value, 0, 0, 0, value} {
}

template <typename T>
inline constexpr Mat<T, 4>::Mat(const ValueType& a11, const ValueType& a22,
                const ValueType& a33, const ValueType& a44) :
    m_storage{a11, 0, 0, 0, 0, a22, 0, 0, 0, 0, a33, 0, 0, 0, 0, a44} {
}

template <typename T>
inline constexpr Mat<T, 4>::Mat(const ValueType& a11, const ValueType& a21,
                const ValueType& a31, const ValueType& a41,
                const ValueType& a12, const ValueType& a22,
                const ValueType& a32, const ValueType& a42,
                const ValueType& a13, const ValueType& a23,
...
template <typename T>
template <std::convertible_to<typename Mat<T, 4>::ValueType> U>
inline constexpr Mat<T, 4>::Mat(const Mat<U, N>& other) :
    m_storage{other.as_vector()} {
}

template <typename T>
template <DenseMatrixExpressionConvertibleTo<
    typename Mat<T, 4>::ValueType> Expr>
inline constexpr Mat<T, 4>::Mat(const Expr& expr) :
...
template <DenseMatrixExpressionConvertibleTo<
    typename Mat<T, 4>::ValueType> Expr>
inline constexpr auto Mat<T, 4>::operator=(const Expr& expr) -> Mat& {
#ifndef NDEBUG
    if (size() != expr.size()) {
...
template <typename T>
inline constexpr auto Mat<T, 4>::operator[](std::size_t column) const
        -> const Vec<T, N>& {
#ifndef NDEBUG
...
template <typename T>
inline constexpr auto Mat<T, 4>::operator[](std::size_t column) -> Vec<T, N>& {
#ifndef NDEBUG
    if (column >= size().second) {
        throw RuntimeError{Range2DError{.indices = {0, column},
            .size = size()}};
...
template <typename T>
inline constexpr auto Mat<T, 4>::operator[](std::size_t row,
...
template <typename T>
inline constexpr auto Mat<T, 4>::operator[](std::size_t row, std::size_t column)
        -> ValueType& {
#ifndef NDEBUG
    if (row >= size().first || column >= size().second) {
        throw RuntimeError{Range2DError{.indices = {row, column},
...
template <typename T>
inline constexpr auto Mat<T, 4>::num_elements() const -> std::size_t {
    return N*N;
}

template <typename T>
inline constexpr auto Mat<T, 4>::size() const
        -> const std::pair<std::size_t, std::size_t>& {
    return m_size;
}

template <typename T>
inline constexpr void Mat<T, 4>::fill(const T& value) {
    m_storage.storage.fill(value);
}

template <typename T>
inline constexpr void Mat<T, 4>::zero() {
...
template <typename T>
template <std::convertible_to<typename Mat<T, 4>::ValueType> U>
inline constexpr auto Mat<T, 4>::operator*=(const U& rhs) -> Mat& {
    for (std::size_t i{0}; i < N*N; ++i) {
        m_storage[i] *= rhs;
    }
    return *this;
...
template <typename T>
inline constexpr auto Mat<T, 4>::data() const -> const ValueType* {
    return m_storage.storage.data();
}

template <typename T>
inline constexpr auto Mat<T, 4>::data() -> ValueType* {
    return m_storage.storage.data();
}

template <typename T>
inline constexpr auto Mat<T, 4>::as_vector() const
