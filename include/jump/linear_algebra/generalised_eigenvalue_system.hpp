...
namespace Jump {
    ...
    class GeneralisedEigenvalueSystem : public LinearSystemBase {
        private:
            ...
            DenseMatrix<T>& m_B;
            /// \brief Reference to the solution data structure, which will
            /// store eigenvalue/eigenvector pairs.
            std::vector<Eigendatum>& m_data;

        public:
            /// \brief Initialise the eigenvalue system with references to the
            /// DenseMatrices in \f$A\vec{x}=\lambda*B*\vec{x}\f$ and the
    ...
    #include "generalised_eigenvalue_system_impl.hpp"
