...
namespace Jump {
    ...
    /// \brief A linear system \f$A\vec{x}=\vec{b}\f$, where \f$A\f$ is a matrix
    /// and \f$\vec{b}\f$ is a right-hand side vector. Upon solving, we have the
    /// solution \f$x\f$.
    template <typename T>
    ...
    #include "dense_linear_system_impl.hpp"
